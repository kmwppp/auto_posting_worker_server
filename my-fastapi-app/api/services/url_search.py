...
async def get_wordpress_post_url(page, site_url, keyword, current_user_id, naver_id, task_id):
    ...
    await redis_manager.publish(task_id, f"🔍 워드프레스에서 '{keyword}' 검색 중...")
    ...
    if not actual_post_url:
        ...
        if fixed_keyword != keyword:
            ...
            await redis_manager.publish(task_id, f"🔄 결과 없음. 키워드 교정 후 재검색 중: '{fixed_keyword}'")
            await asyncio.sleep(0.1)
            actual_post_url = await fetch_url(fixed_keyword)

    # --- 최종 결과 처리 ---
    if actual_post_url:
        await redis_manager.publish(task_id, f"🔗 URL 추출 성공: {actual_post_url}")
        await asyncio.sleep(0.1)
        await log_to_db(current_user_id, naver_id, f"추출 URL: {actual_post_url}", "워드프레스 URL 추출 성공")
        return actual_post_url
    else:
        await redis_manager.publish(task_id, f"⚠️ '{keyword}'에 대한 최종 검색 결과가 없습니다.")
        await asyncio.sleep(0.1)
...
async def get_blogspot_post_url(page, site_url, keyword, current_user_id, naver_id, task_id):
    ...
    await redis_manager.publish(task_id, f"🔍 블로그스팟에서 '{keyword}' 검색 중...")
    ...
    if not actual_post_url:
        ...
        if fixed_keyword != keyword:
            await redis_manager.publish(task_id, f"🔄 결과 없음. BlogSpot 키워드 교정 재검색: '{fixed_keyword}'")
            actual_post_url = await fetch_url(fixed_keyword)

    # 최종 결과 반환
    if actual_post_url:
        await redis_manager.publish(task_id, f"🔗 BlogSpot URL 추출 성공: {actual_post_url}")
        await log_to_db(current_user_id, naver_id, f"추출 URL: {actual_post_url}", "블로그스팟 URL 추출 성공")
        return actual_post_url
    else:
        await redis_manager.publish(task_id, f"⚠️ BlogSpot 검색 결과 없음.")
        return site_url